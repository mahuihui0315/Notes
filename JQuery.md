...
### 祖先
下面jQuery方法用于向上遍历 DOM 树
+ parent()：返回被选元素的父元素
+ parents()：返回被选元素的所有父元素，直到根元素(\<html>)
+ parentsUtil("...")：返回两个给定元素之间的元素

### 后代
下面是两个用于向下遍历 DOM 树的 jQuery 方法
+ children()：返回被选元素的所有直接子元素，也可输入参数进行过滤
> 例如：$("div").children("p.1")，返回div元素下的所有类名为1的p元素
+ find()：返回被选元素的所有后代直到最后一个，也可输入参数进行过滤